...
export interface Config {
  ...
  gain: number;
  /** Absolute servo angle reported by firmware v6. */
  servo: number;
}
    ...
    case 'CFG':
      return {
        ...
        config: {
          ...
          led: rest[1] === '1',
          gain: Number(rest[2]),
          // Firmware v5 had no servo field. Keep its home angle as the
          // fallback so an older connected board still produces valid state.
          servo: Number.isFinite(Number(rest[3])) ? Number(rest[3]) : 90,
        },
      };
...
export const command = {
  ...
  servo: (angle: number) => `SERVO ${Math.max(0, Math.min(180, Math.round(angle)))}`,
};
